...
# Heuristic on full/empty stations -- Not sure if this is useful anymore
...
```{r}
stat.clusters = stations %>% filter(District %in% c("Cambridge", "Boston")) %>%
  select(station.id, Latitude, Longitude)

set.seed(147)
km100 <- kmeans(stat.clusters %>% select(Longitude, Latitude), 
             centers = 100, iter.max=1000) # centers randomly selected from rows of airline.scaled
clusters100 <- km100$cluster
table(clusters100)

set.seed(147)
km50 <- kmeans(stat.clusters %>% select(Longitude, Latitude), 
             centers = 50, iter.max=1000) # centers randomly selected from rows of airline.scaled
clusters50 <- km50$cluster
table(clusters50)
```
